...
Ext.define('Ext.chart.axis.Axis', {
    ...
    getSprites: function () {
        ...
        if (!me.sprites.length) {
            baseSprite = new Ext.chart.axis.sprite.Axis({docked: position, axis: me});
        ...
        if (range) {
            baseSprite.fx.setDuration(0);

            if (title) {
                switch (position) {
                    case 'top':
                        title.setAttributesCanonical({
                            x: surface.roundPixel(length / 2),
                            y: 0,
                            textBaseline: 'top',
                            textAlign: 'center'
                        });
                    ...
                    case 'left':
                        title.setAttributes({
                            x: 0,
                            y: surface.roundPixel(length / 2),
                            textBaseline: 'top',
                            textAlign: 'center',
                            rotation: {
                                centerX: 0,
                                centerY: surface.roundPixel(length / 2),
                                degrees: -90
                            }
                        });
                        title.applyTransformations();
                    ...
                    case 'right':
                        title.setAttributes({
                            x: baseSprite.thickness,
                            y: surface.roundPixel(length / 2),
                            textBaseline: 'top',
                            textAlign: 'center',
                            rotation: {
                                centerX: baseSprite.thickness,
                                centerY: surface.roundPixel(length / 2),
                                degrees: -90
                            }
                        });
            ...
            if (me.getChart().getGridSprite()) {
                baseSprite.bindMarker(me.getChart().getGridSprite());
            }
        }

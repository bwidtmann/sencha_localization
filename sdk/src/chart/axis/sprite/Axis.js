...
Ext.define("Ext.chart.axis.sprite.Axis", {
            ...
            switch (docked) {
                case 'right':
                    for (it = me.iterate(majorTicks); it.hasNext(); it.step()) {
                        position = surface.roundPixel(it.get() * yy + dy);
                ...
                case 'left':
                    for (it = me.iterate(majorTicks); it.hasNext(); it.step()) {
                        position = surface.roundPixel(it.get() * yy + dy);
                        ctx.moveTo(thickness - majorTickSize + titleOffset, position);
                        ctx.lineTo(thickness + titleOffset, position);
                    }
                    break;
                case 'bottom':
                    for (it = me.iterate(majorTicks); it.hasNext(); it.step()) {
                        position = surface.roundPixel(it.get() * xx + dx);
                ...
                case 'top':
                    for (it = me.iterate(majorTicks); it.hasNext(); it.step()) {
                        position = surface.roundPixel(it.get() * xx + dx);
                        ctx.moveTo(position, thickness + titleOffset);
                        ctx.lineTo(position, thickness - majorTickSize + titleOffset);
                    }
                    break;
            }
        }
    ...
    renderLabels: function (surface, ctx, layout, clipRegion) {
        var me = this,
            ...
            yy = matrix.getYY(),
            dy = matrix.getDY(),
            targetThickness = me.thickness + titleOffset,
            thickness = 0,
            majorTicks = layout.majorTicks,
            vertical = docked === 'left' || docked === 'right',
            ...
            switch (docked) {
                case 'left':
                    label.setAttributesCanonical({
                        textAlign: 'right',
                        textBaseline: 'center',
                        translationX: surface.roundPixel(targetThickness - padding + dx)
                    });
                    break;
                case 'right':
                    label.setAttributesCanonical({
                        textAlign: 'left',
                        textBaseline: 'center',
                        translationX: surface.roundPixel(padding + dx)
                    });
                    break;
                case 'top':
                    label.setAttributesCanonical({
                        textAlign: 'center',
                        textBaseline: 'bottom',
                        translationY: surface.roundPixel(targetThickness - padding)
                    });
                    break;
                case 'bottom':
                    label.setAttributesCanonical({
                        textAlign: 'center',
                        textBaseline: 'top',
                        translationY: surface.roundPixel(padding)
                    });
                    break;
            }
    ...
    renderAxisLine: function (surface, ctx, layout, clipRegion) {
        ...
        if (attr.axisLine) {
            switch (docked) {
                case 'left':
                    ctx.moveTo(titleOffset + thickness - 0.5, 0);
                    ctx.lineTo(titleOffset + thickness - 0.5, length + 1);
                    break;
                case 'right':
                    ctx.moveTo(0.5, 0);
                ...
                case 'bottom':
                    ...
                    break;
                case 'top':
                    ctx.moveTo(0, thickness - 0.5 + titleOffset);
                    ctx.lineTo(length + 1, thickness - 0.5 + titleOffset);
                    break;
            }
        }
    },
    ...
    renderGridLines: function (surface, ctx, layout, clipRegion) {
        var me = this,
            ...
            docked = attr.docked,
            majorTicks = layout.majorTicks,
            it, position, lastPosition;
        if (attr.grid) {
            if (majorTicks) {
                if (docked === 'left' || docked === 'right') {
                    lastPosition = attr.min * yy + dy;
                    for (it = me.iterate(majorTicks); it.hasNext(); it.step()) {
                        position = it.get() * yy + dy;
                    ...
                    lastPosition = attr.min * xx + dx;
                    for (it = me.iterate(majorTicks); it.hasNext(); it.step()) {
                        position = it.get() * xx + dx;
                        me.putMarker('vertical', {
                            x: position,
                            width: lastPosition - position
                        });
                        lastPosition = position;
                    }
                }

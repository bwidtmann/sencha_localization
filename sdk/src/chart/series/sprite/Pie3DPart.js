...
Ext.define("Ext.chart.series.sprite.Pie3DPart", {
    ...
    getBBox: function (isWithoutTransform) {
        ...
        if (!bbox.plain) {
            bbox.plain = {
                width: rho * 2,
                height: rho * attr.distortion * 2 + attr.thickness
            };
            bbox.plain.x = attr.centerX - rho;
            bbox.plain.y = attr.centerY - rho * attr.distortion;
        }
        if (isWithoutTransform) {
            return bbox.plain;
        }
        if (!bbox.transform) {
            bbox.transform = attr.matrix.transformBBox(bbox.plain);
        }

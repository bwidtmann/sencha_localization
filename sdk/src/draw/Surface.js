...
Ext.define('Ext.draw.Surface', {
    ...
    renderFrame: function () {
        if (!this.element || this.dirtyPredecessor > 0) {
            return;
        }
        var me = this,
        ...
        if (me.getDirty()) {
            ...
            for (i = 0, ln = items.length; i < ln; i++) {
                ...
                item.attr.textPositionCount = me.textPosition - textPositionStart;
            }
    ...
    waitFor: function (surface) {
        ...
        if (!Ext.Array.contains(predecessors, surface)) {
            ...
            surface.setDirty = function (dirty) {
                if (dirty) {
                    if (!surface._dirty) {
                        me.dirtyPredecessor++;
                        if (!me.dirty) {
                        ...
                        oldSetDirty.call(surface, dirty);
                    }
                } else {
                    if (surface._dirty) {
                        me.dirtyPredecessor--;
